...
```{r}
p2 <- prices %>% 
  filter(Format == 'Hardcover' & BISAC_Level_1 == 'FICTION') %>% 
  group_by(BISAC_Level_2, BISAC_Level_1) %>% 
  mutate(count=n()) #%>% 
  # order() %>% 
  # select(BISAC_Level_1, BISAC_Level_2, count) %>% 
  # # distinct()
...
q                                                                                                                                                                        
cat_plot <- q %>% 
  ggplot(aes(y = BISAC_Level_2, fill = n)) +
